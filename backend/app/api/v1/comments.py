"""评论路由（§5.1，博客互动 → 博客 scope）。"""
from fastapi import APIRouter, Depends

from app.api.deps import get_blog_user_id, get_pagination
...
async def delete_comment(comment_id: int, user_id: int = Depends(get_blog_user_id)):
...
async def toggle_comment_like(
    comment_id: int, user_id: int = Depends(get_blog_user_id)
):
